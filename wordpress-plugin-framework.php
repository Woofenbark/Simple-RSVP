 ...
 * @author Keith Huster
 */
class SimpleRsvp_WordpressPluginFramework
{
   // ---------------------------------------------------------------------------
	...
	function _AddAdministrationAjax()
   {
      ...
      ?>
      <script type='text/javascript' src='<?php echo admin_url('js/postbox.js'); ?>'></script>
      ...
      if( array_key_exists( $optionName, $this->_pluginOptionsArray ) )
      {
         switch( $this->_pluginOptionsArray[$optionName][$this->OPTION_INDEX_TYPE] )
         {
            case $this->OPTION_TYPE_TEXTBOX:
               ...
               //$optionMarkup = '<tr valign="top">';
               //$optionMarkup .= '<th scope="row"><label for="'.$optionName.'">';
               //$optionMarkup .= $this->_pluginOptionsArray[$optionName][$this->OPTION_INDEX_DESCRIPTION];
               //$optionMarkup .= '</label></th>';
               //$optionMarkup .= '<td><input id="'.$optionName.'" class="regular-text" type="text" ';
               //$optoinMarkup .= 'value="'.get_option( $optionName ).'name="'.$optionName.'"/></td></tr>';
               break;
